...
<?php if(has_post_format('image')): ?>
    <?php if ( has_post_thumbnail() ) : ?>
        ...
        <?php else : ?>
            <div class="tp-blog-thumb overflow-hidden mb-35">
                <!-- Blog Thumbnail Dynamic function, Image Post Format Markup  -->
...
<?php elseif(has_post_format('video')):
    // Meta field Function to retrieve the video URL for the Video Post Format.
    // function_exists() checks if tpmeta_field() is available, preventing errors when the plugin is inactive.
    $video_url = function_exists('tpmeta_field') ? tpmeta_field('format_video_url') : '';
    ?>

    <!-- Video Post Format Markup -->
    <?php if ( has_post_thumbnail() ) : ?> 
...
<?php elseif(has_post_format('gallery')):
    // Meta field Function to retrieve the gallery images for the Gallery Post Format.
    // function_exists() checks if tpmeta_gallery_field() is available, preventing errors when the plugin is inactive.
    $gallery_images = function_exists('tpmeta_gallery_field') ? tpmeta_gallery_field( 'format_gallery' ) : '';
    ?>
    
    <!-- Gallery Post Format Markup -->
