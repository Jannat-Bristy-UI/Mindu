...
function mindu_breadcrumb(){ 
		...
		elseif ( is_single() && 'product' == get_post_type() ) {
			$title = get_theme_mod( 'breadcrumb_product_details', __( 'Shop', 'mindu' ) );
		}
		...
		/* ================= SEARCH / ERROR ================= */
		elseif ( is_search() ) {
			$title = esc_html__( 'Search Results for: ', 'mindu' ) . get_search_query();
		}
		elseif ( is_404() ) {
			$title = esc_html__( '404 Page not Found', 'mindu' );
		}
               ...
               <div class="col-12">
                  <div class="tp-breadcrumb-3-content text-center">
                    <?php if( function_exists('tp_breadcrumb')) : ?> 
                     ...
                     <h2 class="tp-breadcrumb-title text-black mb-0 fw-700 wow fadeInUp" data-wow-duration=".9s" data-wow-delay=".3s"><?php echo mc_kses($title);?></h2>
                  </div>
               </div>
            </div>
         </div>
