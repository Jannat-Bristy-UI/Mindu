<?php

class Mindu_Faq extends \Elementor\Widget_Base {

	public function get_name(): string {
		return 'mindu-faq';
	}

	public function get_title(): string {
		return esc_html__( 'FAQ', 'elementor-addon' );
	}

	public function get_icon(): string {
		return 'icon-ehp-cta';
	}

	public function get_categories(): array {
		return [ 'mindu-category' ];
	}

	public function get_keywords(): array {
		return [ 'faq' ];
	}

	protected function register_controls(): void {

		// Content Tab Start

		//======= FAQ Widget  =======
			$this->start_controls_section(
				'section_list',
				[
					'label' => esc_html__( 'FAQ List', 'elementor-addon' ),
					'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
				]
			);

			// Repeater Control
			$repeater = new \Elementor\Repeater();

			$repeater->add_control(
				'title',
				[
					'label' => esc_html__( 'Title', 'elementor-addon' ),
					'type' => \Elementor\Controls_Manager::TEXT,
					'default' => esc_html__( 'Title Here', 'elementor-addon' ),
					'label_block' => true,
				]
			);

			$repeater->add_control(
				'content',
				[
					'label' => esc_html__( 'Content', 'elementor-addon' ),
					'type' => \Elementor\Controls_Manager::TEXTAREA,
					'default' => esc_html__( 'I need to get a certification for English proficiency and Acadia is my best choice.', 'elementor-addon' ),
				]
			);

			
			
			// Items data Store here
			$this->add_control(
				'list',
				[
					'label' => esc_html__( 'FAQ List', 'textdomain' ),
					'type' => \Elementor\Controls_Manager::REPEATER,
					'fields' => $repeater->get_controls(),
					'default' => [
						[
							'list_title' => 'Title #1',
						],

						[
							'list_title' => 'Title #2',
							
						],

					],
				
				]
			);

			$this->end_controls_section();
		//======= End Widget  =======

		// Content Tab End


		// Style Tab Start
		$this->start_controls_section(
			'section_title_style',
			[
				'label' => esc_html__( 'Title', 'elementor-addon' ),
				'tab' => \Elementor\Controls_Manager::TAB_STYLE,
			]
		);

		$this->add_control(
			'title_color',
			[
				'label' => esc_html__( 'Text Color', 'elementor-addon' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .hello-world' => 'color: {{VALUE}};',
				],
			]
		);

		$this->end_controls_section();
		// Style Tab End

	}

	protected function render(): void {
		$settings = $this->get_settings_for_display();

		?>

		<div class="tp-faq mb-30">
			<div class="accordion" id="accordionExample">

				<?php foreach ($settings['list'] as $key => $item) : 
					$collapsed = ($key === 0) ? '' : 'collapsed';
					$show = ($key === 0) ? 'show' : '';
					$active = ($key === 0) ? 'active' : '';
				?>
					<div class="tp-faq-item <?php echo esc_attr($active); ?> wow fadeInUp" data-wow-duration=".9s" data-wow-delay=".3s">
						<h2 class="accordion-header">
							<button class="tp-faq-button <?php echo esc_attr($collapsed); ?>" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne-<?php echo esc_attr($key); ?>" aria-expanded="true" aria-controls="collapseOne">
								<?php echo esc_html($item['title']); ?>
							</button>
						</h2>
						<div id="collapseOne-<?php echo esc_attr($key); ?>" class="tp-faq-collapse collapse <?php echo esc_html($show); ?>" data-bs-parent="#accordionExample">
							<div class="tp-faq-body">
								<p><?php echo esc_html($item['content']); ?></p>
							</div>
						</div>
					</div>
				<?php endforeach; ?>
			</div>
    	</div>


		<?php

	}

}

$widgets_manager->register( new Mindu_Faq());

